...
OUTPUT_NAME="elora"                                   # Output file name (no ext.)

# ==== MAIN ====
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
KEYMAP_FILE="$SCRIPT_DIR/keymap.json"
TMP_KEYMAP_FILE="$SCRIPT_DIR/keymap-tmp.json"
DRAWINGS_DIR="docs/generated/elora"
...
# Prepare keymap
jq '.layout = "LAYOUT_split_4x6_5" | .layers = (.layers | map(.[:-10]))' "$KEYMAP_FILE" >"$TMP_KEYMAP_FILE"

# Generate intermediate YAML
uv run keymap -c "$CONFIG_PATH" parse -c "$COLS" -l $LAYERS_STR -q "$TMP_KEYMAP_FILE" >"$YAML_FILE"
